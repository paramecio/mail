...
"""
    ...
    if call("sudo useradd -m -s /usr/sbin/nologin -d /home/%s %s" % (args.domain, args.user),  shell=True, stdout=DEVNULL) > 0: 
        json_return['error']=1
        json_return['status']=1
        json_return['progress']=100
        json_return['message']='Error: cannot create a new user'
    ...
    if args.quota>0:
        num_m=args.quota*1024
        num_mhard=(num_m*0.10)+num_m
        if call("sudo setquota -g %s -a %d %d 0 0" % (args.user, num_m, num_mhard),  shell=True, stdout=DEVNULL) > 0:
            json_return['error']=1
            json_return['status']=1
            json_return['progress']=100
            json_return['message']='Error: cannot add quota to this user group'
