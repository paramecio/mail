...
class MailBox(WebModel):
    
    def __init__(self, connection):
        
        super().__init__(connection)
        self.register(EmailField('mailbox'), True)
        self.register(corefields.ForeignKeyField('domain_id', DomainMail(connection), size=11, required=True, identifier_field='id', named_field="domain", select_fields=['quota']))
